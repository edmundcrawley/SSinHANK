...
#        inc = {'labor': 0.9*np.ones((self.mpar['nm'],self.mpar['nh'])) } # initial labor income for natural borrowing const
        ...
        c_guess = Results_excessB['c_policy']
    ...
    def excessB_update(self, SS_return, grid,P_H,mpar,par,meshes):
        ...
        return{'excess':ExcessA,'c_policy':c_guess,'m_policy':m_star,'joint_distr':joint_distr,
               'W_fc':W_fc,'Profits_fc':Profits_fc,'Output':Output,'N':N,'par':par,'grid':grid,'inc':inc}
...
if __name__ == '__main__':
    ...
    pickle.dump(EX2SS, open("SS_fsolve.p", "wb"))
    
    EX2SS=pickle.load(open("SS_fsolve.p", "rb"))