...
class FluctuationsOneAssetIOUsBond:
    ...
    def __init__(self, par, mpar, grid, Output, targets, Vm, joint_distr, Copula, c_new,c_policy, m_policy, mutil_c, P_H,inc,Profits_fc,C_agg,L_new):
    ...
    def StateReduc(self):
        ...
        Yss=np.vstack(( invmutil(np.reshape(self.mutil_c.copy(),(np.product(self.mutil_c.shape),1),order='F')),
                       ...
                       np.log(self.targets['N']),np.log(self.targets['B']),np.log(self.C_agg) ))
...
def plot_IRF(mpar,par,gx,hx,joint_distr,Gamma_state,grid,targets,os,oc,Output,c_new,
...
#    IRF_G=100*IRF_state_sparse[-1-oc+7, :-1]/targets['Y']   # write IRF_G in terms of percentage of output
    ...
    IRF_B=100*IRF_state_sparse[-1-oc+6, :-1]    
    IRF_Cagg = 100*IRF_state_sparse[-1-oc+7, :-1]
...
#    IRF_RBREAL=IRF_RB-IRF_PI
    ...
    for i in range(0,mpar['maxlag']-1):
        labor_income_aux[:,:,i] = labor_aux[0,i]*np.ones((mpar['nm'],mpar['nh']))
    
    labor_income_aux[:,-1,:]=0
        
    profit_aux = IRF_Profit    
    profit_income_aux = np.zeros((mpar['nm'],mpar['nh'],mpar['maxlag']-1))    
    
    for i in range(0,mpar['maxlag']-1):
        profit_income_aux[:,:,i] = profit_aux[0,i]*np.ones((mpar['nm'],mpar['nh']))    
    
    profit_income_aux[:,0:-1,:]=0
    
    IRF_Yi = labor_income_aux + profit_income_aux
    
    jd_aux = np.tile(joint_distr,(mpar['maxlag']-1))
    jd_series = np.reshape(np.array(jd_aux),(mpar['nm'],mpar['nh'],mpar['maxlag']-1),order='F')
    E_IRF_Yi = (np.sum(np.sum(np.multiply(jd_series,IRF_Yi),0),0))   


    E_Y = np.sum(np.sum(np.multiply(joint_distr,WW_h_mesh)))
    MPC_over_t = np.tile(MPC_m,mpar['maxlag']-1).reshape(mpar['nm'],mpar['nh'],mpar['maxlag']-1,order='F')

    IRF_Y_series = np.zeros((mpar['nm'],mpar['nh'],mpar['maxlag']-1))
    
    for i in range(0,mpar['maxlag']-1):
        IRF_Y_series[:,:,i] = IRF_Y[0,i]*np.ones((mpar['nm'],mpar['nh']))
...
#    IRF_GHH = Add1_C * IRF_W 
    ...
    IRF_C_by_suff =  IRF_AggInc +  IRF_EarnHet + IRF_Fisher + IRF_IRE + IRF_IntSubs
...
#     recall that IRF_PI and IRF_RBREAL are in bp term!
    ...
    print('M: ', str(M))
    print('E(Epsilon_Y):',str(np.mean(Earning_hetero)))
    print('Epsilon_P:', str(Redist_elas_P))    
    print('Epsilon_R:', str(Redist_elas_R))
    print('Hicks_scaling_factor:', str(Hick_scaling))
...
#    f_X.show()   
    ...
    return{'IRF_state_sparse': IRF_state_sparse, 'IRF_C_by_suff':IRF_C_by_suff,
           'IRF_Cagg':IRF_Cagg, 'IRF_W':IRF_W, 'IRF_N':IRF_N,
           'IRF_Profit':IRF_Profit, 'IRF_RB':IRF_RB, 'IRF_RBREAL':IRF_RBREAL, 'IRF_PI':IRF_PI,
           'IRF_AggInc':IRF_AggInc,  'IRF_EarnHet':IRF_EarnHet, 'IRF_Fisher':IRF_Fisher,
           'IRF_IRE':IRF_IRE, 'IRF_IntSubs':IRF_IntSubs}
...
#    invmutil = lambda x : (1./x)**(1./par['xi'])
    ...
    LHS = np.zeros((nx+Cind+1,1))
    RHS = np.zeros((nx+Cind+1,1))
    ...
    ciminus = np.asarray(np.reshape(invmutil(mutil_cminus),[mpar['nm'],mpar['nh']]))
    
    meshes_E = np.hstack((meshes['h'][:,0:-1],np.zeros((mpar['nm'],1))))
    Niminus = (np.multiply(ciminus**(-par['xi']),meshes_E/par['H'])*Wminus)**(1/par['gamma'])

    WW = Wminus*np.reshape(Niminus,[mpar['nm'],mpar['nh']])
...
#    inc = {'labor':inclabor, 'money':incmoney, 'profits':incprofits}
    ...
    result_EGM_policyupdate = EGM_policyupdate(EVm,PIminus,RBminus,inc,meshes,grid,par,mpar,Hminus,Wminus,Profitminus)
    c_star = result_EGM_policyupdate['c_star']
    m_star = result_EGM_policyupdate['m_star']
    

    RHS[nx+Cind] = np.sum(np.sum(np.multiply(JDminus,c_star)))
    ...
    L_new = (np.multiply(c_star**(-par['xi']),meshes_E/par['H'])*Wminus)**(1/par['gamma'])

    RHS[nx+Nind] = np.sum(np.sum( np.multiply(L_new,JDminus) ))
    RHS[nx+Yind] = (TFP*np.asarray( np.sum(np.sum(np.multiply(L_new,JDminus))) )**par['alpha']*grid['K']**(1-par['alpha']))
    
    # Wage Rate
    RHS[nx+Wind] = TFP * par['alpha'] * mc *(grid['K']/np.asarray( np.sum(np.sum(np.multiply(L_new,JDminus))) ))**(1-par['alpha'])
...
def EGM_policyupdate(EVm,PIminus,RBminus,inc,meshes,grid,par,mpar,Hminus,Wminus,Profitminus):
    ...
    mmin = grid['m'][0]
    RBminus = np.asarray(RBminus.copy())
    PIminus = np.asarray(PIminus.copy())
    cpbind = ((RBminus/PIminus)*meshes['m']+mmin)/2+np.sqrt(((RBminus/PIminus)*meshes['m']+mmin)**(2)+4*(meshes['h']/par['H']*Wminus)**(1+1/par['gamma']))/2
    cpbind[:,-1] = ((RBminus/PIminus)*meshes['m'][:,-1]+mmin+Profitminus*par['profitshare']);    
...
#    m_n_aux = (c_new.copy() + meshes['m'].copy()-inc['labor'].copy()-inc['profits'].copy())
    m_n_aux = (c_new.copy() + meshes['m'].copy()-inc['labor'].copy())/(RBminus/PIminus)
...
if __name__ == '__main__':
    ...
    EX2SS=pickle.load(open("SS_fsolve_sep_nm50_eqdist.p", "rb"))
    ...
    plot_IRF(SR['mpar'],SR['par'],SGUresult['gx'],SGUresult['hx'],SR['joint_distr'],
         SR['Gamma_state'],SR['grid'],SR['targets'],SR['os'],SR['oc'],SR['Output'],c_new,
         WW_h_mesh,MPC_m,Inc_wt_MPC,Redist_elas_P,Redist_elas_R,Hick_scaling,URE,NNP)
