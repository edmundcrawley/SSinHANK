...
class SteadyState_fsolve:
    ...
    def SolveSteadyState(self):
        ...
        fex = lambda Guess : self.excessB(Guess,grid,P_H,self.mpar,self.par,meshes)
        
        Guess = par['RB']
        SS_Return = fsolve(fex,Guess)
        par['RB']=SS_Return

        Results_excessB = self.excessB_update(SS_Return,grid,P_H,self.mpar,self.par,meshes)
        ...
        tax = (1.-par['tau'])*W_fc*grid['N'] +(1.-par['tau'])*Profits_fc
    ...
    def FactorReturns(self, meshes, grid, par, mpar,L_guess):
...
#        Y = N**par['alpha']*grid['K']**(1-par['alpha'])
    ...
    def PolicyGuess(self, meshes, WW, RBRB, par, mpar,grid, W_fc, P_H, Profits_fc,Output):
...
#        c_guess = inc['labor'].copy()+np.maximum(inc['money'],0.) + inc['profits']
    ...
    def PoliciesSS(self,c_guess, grid, inc, RBRB, P, mpar, par, meshes, Profits_fc, W_fc):
        ...
        while np.max((distC)) > mpar['crit']:
               ...
               resultEGM = self.EGM(grid, inc, money_expense, c_aux, mpar, par, meshes, Profits_fc, W_fc)
    ...
    def EGM(self, grid, inc, money_expense, c_aux, mpar, par, meshes, Profits_fc, W_fc):
...
#        binding_constraints = c_update > Resource
    ...
    def excessB(self, Guess, grid,P_H,mpar,par,meshes):
        ...
        while np.max((dist_L)) > mpar['crit']:
             ...
             resultFactReturn = self.FactorReturns(meshes, grid, par, self.mpar,L_guess)
             ...
             resultPolGuess = self.PolicyGuess(meshes,WW,RBRB,par,self.mpar,grid,W_fc,P_H,Profits_fc,Output)
             ...
             resultPolicySS = self.PoliciesSS(c_guess, grid, inc, RBRB, P_H, self.mpar, par, meshes, Profits_fc, W_fc)
    ...
    def excessB_update(self, SS_return, grid,P_H,mpar,par,meshes):
...
if __name__ == '__main__':
    ...
    EX2 = SteadyState_fsolve(**EX2param)
    
    EX2SS = EX2.SolveSteadyState()
    
    pickle.dump(EX2SS, open("SS_fsolve_sep_nm100_nh_6_eqdist.p", "wb"))
    
    EX2SS=pickle.load(open("SS_fsolve_sep_nm100_nh_6_eqdist.p", "rb"))