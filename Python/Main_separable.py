...
import pickle
from SteadyState_fsolve_separable import SteadyState_fsolve
from FluctuationsOneAssetIOUsBond_separable import FluctuationsOneAssetIOUsBond, Fsys, SGU_solver, plot_IRF
...
SSS=pickle.load(open("SS_fsolve_sep_nm50_eqdist.p", "rb")) # Use old steady state
...
#    MPC_m[:,hh]=np.gradient(np.squeeze(C_ind[:,hh]))/np.gradient(grid['m'])  # MPC_m_ is same with MPC_m
    MPC_m[:,hh]=np.gradient(np.squeeze(c_new[:,hh]))/np.gradient(grid['m'])  # MPC_m_ is same with MPC_m
...
NNP = meshes_m
URE = inc['labor'] + meshes_m - c_new + inc['profits']
...
sig_i = par['xi']**(-1)
Hick_scaling = np.sum(np.sum( np.multiply(np.multiply(np.multiply(sig_i, (1-MPC_m.copy())), c_new.copy()), jd.copy()) ))
...
print(round(plotresult['IRF_Cagg'][0,0],3))
