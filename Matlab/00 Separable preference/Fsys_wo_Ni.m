function [Difference,LHS,RHS,JD_new,c_star,m_star,P]  = Fsys_wo_Ni(State,Stateminus,...
...
ciminus = reshape(invmutil(mutil_cminus),[mpar.nm,mpar.nh]);
Niminus = (ciminus.^(-par.xi).*[meshes.h(:,1:end-1)/Hminus,zeros(mpar.nm,1)]*Wminus).^(1/par.gamma);
...
L_new = (c_star.^(-par.xi).*[meshes.h(:,1:end-1)/Hminus,zeros(mpar.nm,1)]*Wminus).^(1/par.gamma);

RHS(nx+Nind)    =  L_new(:)'*JDminus(:);

RHS(nx+Yind)    = (TFP*(L_new(:)'*JDminus(:)).^(par.alpha).*grid.K.^(1-par.alpha));

% Wage Rate
RHS(nx+Wind)    = TFP *par.alpha*mc.* (grid.K./(L_new(:)'*JDminus(:))).^(1-par.alpha);
