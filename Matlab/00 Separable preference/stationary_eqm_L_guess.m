...
L_guess(:,end)=0;
Y=sum(sum(L_guess))/sum(sum(ones(mpar.nm,mpar.nh)));
pf_guess=(1-mc)*Y;
% pf_guess=(1-mc);
...
while (distC>crit)
 ...
 [c_new,m_star]=EGM_Step1_a_guess(grid,inc,money_expense,c_aux,mpar,par,meshes,w0,mmin,pf_guess,iter);
...
% L  = @(c,h,w) invvp(up(c).*h.*w);
 ...
 L_new = L(c_aux,[meshes.h(:,1:end-1)/par.H,zeros(mpar.nm,1)],w0); % Es do not supply labor
