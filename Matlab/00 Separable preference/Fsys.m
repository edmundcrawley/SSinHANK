function [Difference,LHS,RHS,JD_new,c_star,m_star,P]  = Fsys(State,Stateminus,...
    Control_sparse,Controlminus_sparse,StateSS,ControlSS,Gamma_state,...
    Gamma_control,InvGamma,Copula,par,mpar,grid,P,aggrshock,oc,os)
% ControlSS=Yss;StateSS=Xss;
% Stateminus=State_m; P=P_H;
...
Nind  = (NN+NN_L)+5;
Bind  = (NN+NN_L)+6;
Cind =  (NN+NN_L)+7; % final goods consumption
...
RHS(nx+Profitind) = (1-mc)*Yminus - Yminus.*(1/(1-par.mu))./par.kappa./2 .*log(PIminus).^2;

%% Wages net of leisure services
WW=Wminus*reshape(Niminus,[mpar.nm,mpar.nh-1]);
WW=[WW,ones(mpar.nm,1)*Profitminus*par.profitshare];
...
[c_star,m_star] = EGM_policyupdate(EVm,PIminus,RBminus,inc,meshes,grid,par,mpar,Hminus,Wminus,Profitminus);

RHS(nx+Cind) = JDminus(:)'*c_star(:);
...
% L_new = L(c_star,[meshes.h(:,1:end-1)/Hminus,zeros(mpar.nm,1)],mc*TFP);
L_new = (c_star(:,1:end-1).^(-par.xi).*meshes.h(:,1:end-1)/Hminus*Wminus).^(1/par.gamma);
...
JDminus_L = JDminus(:,1:end-1);

RHS(nx+Nind)    =  L_new(:)'*JDminus_L(:);

% RHS(nx+Yind)    = (TFP*(Nminus).^(par.alpha).*grid.K.^(1-par.alpha));
RHS(nx+Yind)    = (TFP*(L_new(:)'*JDminus_L(:)).^(par.alpha).*grid.K.^(1-par.alpha));

% Wage Rate
RHS(nx+Wind)    = TFP *par.alpha*mc.* (grid.K./(L_new(:)'*JDminus_L(:))).^(1-par.alpha);
